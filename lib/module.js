...
module.exports = function (moduleOptions) {
  ...
  this.extendBuild((config, { isDev }) => {
    ...
    if (config.name === 'client') {
      ...
      config.plugins = [
        ...config.plugins,
        new WrapperPlugin({
          test: /\.ilc\.js$/, // only wrap output of bundle files with '.ilc.js' extension
  ...
  this.addTemplate({
    src: resolve(__dirname, 'views/error.html'),
  ...
  this.nuxt.hook('webpack:config', (webpackConfigs) => {
    ...
    clientConfig.entry.app.some((src, idx) => {
      ...
      clientConfig.entry.app[idx] = src
        .replace('path=', 'path=' + hotPublicPath)
      return true
    })
  })
